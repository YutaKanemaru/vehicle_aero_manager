...
function defaultValues(template: TemplateResponse) {
  ...
  return {
    ...
    activate_body_tg: so?.boundary_condition?.turbulence_generator?.activate_body_tg ?? true,
    activate_ground_tg: so?.boundary_condition?.turbulence_generator?.activate_ground_tg ?? true,
  };
}
...
export function TemplateVersionCreateModal({ opened, onClose, template }: Props) {
  ...
  function handleSubmit(values: typeof form.values) {
    const payload: TemplateVersionCreate = {
      ...
      settings: {
        ...
        setup: template.active_version?.settings?.setup ?? {
        ...
        target_names: template.active_version?.settings?.target_names ?? {
          wheel: [],
          rim: [],
          porous: [],
          car_bounding_box: [],
