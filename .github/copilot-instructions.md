...
- `transform_run(db, case_id, run_id, current_user, background_tasks)` — derives all transform params from Run's Condition + Case (Template/Assembly); calls `ride_height_service.compute_transform()` + `create_system_and_geometry()`; patches `geometry_override_id` on the Run; returns dict with `system_id`, `geometry_id`, `geometry_name`, `geometry_status="pending"`, `transform_snapshot`. **Returns within milliseconds** — STL write + analysis + GLB generation all run in background task.
...
- `src/components/cases/CaseDetailPage.tsx` — dedicated page at `/cases/:caseId`; 2 tabs:
  ...
  - **Runs** tab: compact run table; per-run **"Apply Transform"** button (`IconTransform`, teal, shown when `needs_transform && !transform_applied && (pending|error)`) triggers `POST /runs/{id}/transform` → on success: `addJob(geometry_id, geometry_name, "stl_analysis")` + `updateJob(geometry_id, "pending")` so progress is visible in Jobs Drawer; per-run **"Generate XML"** button (`IconPlayerPlay`, disabled when `needs_transform && !transform_applied`); **"Transform All"** + **"Generate All"** bulk buttons; **"Transform All"** uses `Promise.allSettled()` (parallel, non-aborting) — each successful result adds geometry to Jobs store (`addJob` + `updateJob("pending")`), each failure shows individual error notification; per-run Download XML (`{run.name}.xml`) / Download STL (`{run.name}.stl`, fetch+blob with auth) / Reset / Delete actions; **"Open 3D Viewer"** button (`IconExternalLink`, violet, `status === "ready"` only) opens fullscreen overlay
...
| Meshing | **Accordion**: General (coarsest voxel, num RL, transition layers) · Triangle Splitting (global ON/OFF switch → when ON: max rel/abs edge + per-part instances) · Box Refinement (dynamic list; each row: name/level + `SegmentedControl` for `box_type`: vehicle_bbox_factors/user_defined — **`around_parts` entries excluded from form list; porous box auto-generated by `buildSettings()` from BC tab switches as `Box_Porous_RL7` / level 7 / parts `["Porous_"]` / offsets 0**) · Offset Refinement (dynamic list, "Apply body defaults") · Custom Refinement (dynamic list) |
| Boundary Conditions | **Accordion**: Flow Domain Configuration (ground height definition + domain bbox factors) · Ground Condition (ground mode select + wheel/rim part names [hidden when `ground_mode="static"`] + OSM parts [hidden when static or full_moving] + BL suction; aero has Select, non-aero has simple BL suction only) · Belt Configuration (aero only, **hidden when `ground_mode="static"`**; isBelt5: belt settings + **required** tire part names tn_wt_*; isBelt1: belt size) · Turbulence Generator (aero only; enable switches + per-TG: `intensity` + `length scale (m)` [placeholder="auto", auto = 4 × RL6 voxel size; `num_eddies` hardcoded 800, not in UI]) · **Porous Media Coefficients** (accordion top: "Porous Box Refinement" section — `box_refinement_porous` Switch + conditional `box_refinement_porous_per_coefficient` Switch; `buildSettings()` auto-generates `Box_Porous_RL7` / level 7 / parts `["Porous_"]` / offsets 0 m when enabled; then porous_coefficients dynamic list) |
...
- `create_system_and_geometry(db, source_geometry, transform_snapshot, name, current_user, condition_id, background_tasks, decimation_ratio=0.05) -> (System, Geometry)` — creates destination directory, creates result `Geometry` row (`is_linked=False`, status=`pending`, `file_size=0`), creates `System` record, then **immediately returns** (no STL I/O on HTTP thread). Schedules `_transform_and_analyze_task()` as background task.
- `_transform_and_analyze_task(db, geometry_id, source_path, dest_path, body_transform, wheel_part_transforms, wheel_patterns, decimation_ratio) -> None` — background task that combines: (1) `_transform_stl_buffered()` (STL write + updates `file_size`), (2) `analyze_stl_to_json()` → sets `analysis_result` + status=`ready-decimating`, (3) `build_viewer_glb()` → status=`ready`. On error at any step: sets `status="error"` + `error_message`.
...
- `runsApi.download(caseId, runId) -> Promise<Blob>`: fetches `/cases/{id}/runs/{id}/download` with auth header → Blob. Used by `downloadXml()` helper in `CaseDetailPage` — saved as `{run.name}.xml`.
- `runsApi.downloadStl(caseId, runId) -> Promise<Blob>`: fetches `/cases/{id}/runs/{id}/download-stl` with auth header → Blob. Used by `downloadStl()` helper in `CaseDetailPage` — saved as `{run.name}.stl`. (replaces old bare `<a href>` which lacked JWT auth)
...
- `<CameraFitter>`: no props; reads `glbLoaded` from store; two `useEffect` hooks:
  ...
  - **`[glbLoaded=true]`**: calls `threeScene.updateMatrixWorld(true)` then `buildGLBBox(scene)` → `maxDim = max(x,y,z)`; iso position `(center + maxDim×1.2, center − maxDim×1.2, center + maxDim×0.6)` (≈8m standoff for 5m vehicle); sets `near/far`; sets `fitted.current = true`. **No `setTimeout`. Does NOT call `setGlbLoaded(false)` — Loading overlay stays hidden after fit.**
