...
- `src/components/cases/CaseDetailPage.tsx` — dedicated page at `/cases/:caseId`; 2 tabs:
  ...
  - **Runs** tab: compact run table; per-run **"Launch Viewer"** button (`IconExternalLink`, violet, `status === "ready"` only) opens `/cases/:id/runs/:id/viewer` in a new tab; "Generate All" button; per-run Generate/Download/Reset/Delete actions
