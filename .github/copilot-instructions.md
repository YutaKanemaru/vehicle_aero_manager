...
- `<FitToPartController>`: watches `fitToTarget` store value → **Perspective**: moves camera toward target **preserving current viewing angle** (direction = `normalize(camera.pos − oldTarget)`); **Orthographic**: keeps camera position, sets `camera.zoom = currentHalfH / radius × 0.8` so the part fills ~80% of viewport height; both modes copy `controls.target` to part center; clears after apply
